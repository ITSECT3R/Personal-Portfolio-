...
export function useCertificationFilters() {
  const [filter, setFilter] = useState<CertificationFilterState>(DEFAULT_CERT_FILTER);

  const allIssuers = useMemo(
    () =>
      ([...new Set(certificationList.map(c => c.issuer))] as CertificationIssuer[]).sort((a, b) =>
        ISSUER_LABEL_MAP[a].localeCompare(ISSUER_LABEL_MAP[b]),
      ),
    [],
  );
  ...
  const allDomains = useMemo(
    () =>
      ([...new Set(certificationList.map(c => c.domain))] as CertificationDomain[]).sort((a, b) =>
        DOMAIN_LABEL_MAP[a].localeCompare(DOMAIN_LABEL_MAP[b]),
      ),
    [],
  );

  const allTechnologies = useMemo(
    (): string[] => [...new Set(certificationList.flatMap(c => c.technologies))].sort(),
    [],
  );
